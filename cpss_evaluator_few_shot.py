"""
CPSS (Creative Product Semantic Scale) 自动化评估脚本 (含 Few-Shot 支持)
...
def load_few_shots(few_shots_path: str | None = None) -> dict:
    path = few_shots_path or _FEW_SHOTS_PATH
    if not os.path.exists(path):
        print(f"[WARN] Few-shots file not found at {path}. Proceeding zero-shot.")
        return {}
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
async def evaluate_single_question(
    ...
    for shot in few_shots:
        shot_prompt = USER_PROMPT_TEMPLATE.format(
            topic=shot["topic"],
            idea_content=shot["idea_content"],
            left_word=item["left"],
            right_word=item["right"],
        )
        messages.append({"role": "user", "content": shot_prompt})
        messages.append({"role": "assistant", "content": str(shot["score"])})

    # 当前实际要评估的问题
    target_prompt = USER_PROMPT_TEMPLATE.format(
        topic=topic,
...
async def evaluate_file(
    ...
    eval_key = f"cpss_evaluation_per_agent_{inference_config['model']}_few_shot"
    data[eval_key] = {}
    file_all_ok = True
    ...
    for agent_name, payload in agent_ideas_dict.items():
        ...
        tasks = [
            evaluate_single_question(
                ...
                topic=topic,
                idea_content=idea_content,
                few_shots=all_few_shots.get(item["key"], []), # 获取对应题目的 3 个 shots
                semaphore=semaphore,
